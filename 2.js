let myData = `13-14 f: ffffffffnfffvv
...
let myPattern = /\n/;
let myArray = myData.split(myPattern);
//myArray is an array containing each complete entry
...
for (let i=0; i<myArray.length; i++) {
    ...
    for (let j=0; j<myArray[i].length; j++) {
        //this loop creates an array of the 'required characters'
        if (myArray[i][j] === `:`) {
            arrayOfCharacters.push(myArray[i][j -1])
        }
    }
    for (let k=0; k<myArray[i].length; k++) {
        //this loop is generating an array of arrays, containing the rules for the minimum and maximum number of times each required character must occur
        if (myArray[i][k] === `-`) {
            if (k === 1) {
                tempArray.push(parseInt(myArray[i][0]))
                //arrayOfArrays.push(tempArray);
            }
            else {
                tempString+=myArray[i][0]
                tempString+=myArray[i][1]
                tempArray.push(parseInt(tempString))
                //arrayOfArrays.push(tempArray)
            }
            if (myArray[i][k+2] === ` `) {
                tempArray.push(parseInt(myArray[i][k+1]))
                arrayOfArrays.push(tempArray);
            }
            else {
                tempString = ``
                tempString+=myArray[i][k+1]
                tempString+=myArray[i][k+2]
                tempArray.push(parseInt(tempString))
                arrayOfArrays.push(tempArray);
            }
...
for (let g=0; g<myArray.length; g++) {
    ...
    let minChecker = arrayOfArrays[g][0];
    let maxChecker = arrayOfArrays[g][1];
    let characterCount = -1;

    for (let t=0; t<myArray[g].length; t++) {
        if (myArray[g][t] === characterChecker) {
            characterCount = characterCount + 1;
        }
    }
    if (characterCount >= minChecker && characterCount <= maxChecker) {
        count = count + 1;
    }
}
